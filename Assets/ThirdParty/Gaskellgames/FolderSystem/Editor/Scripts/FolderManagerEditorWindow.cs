...
namespace Gaskellgames.FolderSystem.EditorOnly
{
    ...
    public class FolderManagerEditorWindow : GgEditorWindow
    {
        ...
        protected override void OnPageGUI()
        {
            ...
            float remainingPageHeight = (drawBanner ? pageHeight - bannerHeight : pageHeight) - (standardSpacing * 3);
            float rightPageWidth = 175;
            float leftPageWidth = pageWidth - (rightPageWidth + ((singleLineHeight + standardSpacing) * 4));
            EditorGUILayout.BeginHorizontal();
            
            // page left (icons)
            EditorGUILayout.BeginVertical(EditorStyles.helpBox, GUILayout.Width(leftPageWidth), GUILayout.MaxHeight(remainingPageHeight));
            OnLeftPageGUI(leftPageWidth);
            EditorGUILayout.EndVertical();
            
            // page right (names)
            EditorGUILayout.BeginVertical(EditorStyles.helpBox, GUILayout.Width(rightPageWidth), GUILayout.MaxHeight(remainingPageHeight));
            OnRightPageGUI();
            EditorGUILayout.EndVertical();
        ...
        private void OnLeftPageGUI(float leftPageWidth)
        {
            // cache defaults
            ...
            float spaceForIcons = leftPageWidth;
            float singleIconSpace = iconSize + (standardSpacing * 1.7f);
            int maxColumns = Mathf.FloorToInt(spaceForIcons / singleIconSpace);
            ...
            for (int row = 0; row < totalRows; row++)
            {
                ...
                for (int column = 0; column < totalColumns; column++)
                {
                    ...
                    {
                        Debug.Log($"{pageWidth} | {spaceForIcons} / {singleIconSpace} = {maxColumns}");
