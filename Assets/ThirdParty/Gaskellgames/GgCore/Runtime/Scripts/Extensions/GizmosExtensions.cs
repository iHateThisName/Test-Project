...
namespace Gaskellgames
{
    ...
    public static class GizmosExtensions
    {
        ...
        /// <summary>
        /// Draws the axis vectors for a given transform.
        /// </summary>
        /// <param name="point"></param>
        /// <param name="size"></param>
        public static void DrawAxis(Transform point, float size)
        {
            if (point == null) { return; }
            
            Vector3 origin = point.position;
            Vector3 up = origin + (point.up * size);
            Vector3 forward = origin + (point.forward * size);
            Vector3 right = origin + (point.right * size);
            
            Gizmos.color = new Color32(115, 201, 061, 255);
            Gizmos.DrawLine(origin, up);
            
            Gizmos.color = new Color32(068, 137, 235, 255);
            Gizmos.DrawLine(origin, forward);
            
            Gizmos.color = new Color32(187, 057, 034, 255);
        ...
        /// <param name="size"></param>
        public static void DrawAxis(Vector3 point, Vector3 forward, Vector3 up, Vector3 right, float size)
        {
            Gizmos.color = new Color32(115, 201, 061, 255);
            Gizmos.DrawLine(point, point + (up.normalized * size));
            
            Gizmos.color = new Color32(068, 137, 235, 255);
            Gizmos.DrawLine(point, point + (forward.normalized * size));
            
            Gizmos.color = new Color32(187, 057, 034, 255);
            Gizmos.DrawLine(point, point + (right.normalized * size));
        }
